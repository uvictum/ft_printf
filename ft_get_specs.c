...
/*   Updated: 2018/01/29 14:44:56 by vmorguno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline int		ft_get_flags(const char *s, t_specs *specs, int len)
{
	...
	if (specs->type == 11)
		specs->flags.hash = 1;
	return (i);
	//define getting "-" in any place in format string
}

static inline int		ft_get_type(const char *s, t_specs *specs)
{
	int				i;
	char			flags[16] = "sSdDioOuUxXpcC%";
	char			*buf;
	...
	return (i + 1);
	//define behaviour in case of type specificator absence
}
